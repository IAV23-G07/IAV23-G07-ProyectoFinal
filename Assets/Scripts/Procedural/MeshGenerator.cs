...
public static class MeshGenerator{
    ...
    public static void GenerateTerrainMeshChunk(Cell[,] mapaCells, Vector2 pos, GameObject basee, float heightPerBlock,float sizePerBlock)
    {
        ...
        for (int y = (int)pos.y * chunkSize; y < (int)(pos.y + 1) * chunkSize && y < size; y++)
        {
            for (int x = (int)pos.x*chunkSize; x < (int)(pos.x+1)*chunkSize && x<size; x++)
            {
                ...
                Vector3 a = new Vector3(topLeftX + x - sizePerBlock, currHeight, topLeftZ - y + sizePerBlock);
                Vector3 b = new Vector3(topLeftX + x + sizePerBlock, currHeight, topLeftZ - y + sizePerBlock);
                Vector3 c = new Vector3(topLeftX + x - sizePerBlock, currHeight, topLeftZ - y - sizePerBlock);
                Vector3 d = new Vector3(topLeftX + x + sizePerBlock, currHeight, topLeftZ - y - sizePerBlock);
        ...
        DrawTextureChunk(mapaCells,pos,renderer);

    }
    public static void DrawTextureChunk(Cell[,] mapaCells, Vector2 pos, MeshRenderer renderer)
    {
        int size = mapaCells.GetLength(0);
    ...
    public static void DrawEdgesChunk(Cell[,] mapaCells, Vector2 pos, GameObject edges, float heightPerBlock,float sizePerBlock)
    {
        int size = mapaCells.GetLength(0);
            ...
            for (int x = (int)pos.x * chunkSize; x < (int)(pos.x + 1) * chunkSize && x < size; x++)
            {
                ...
                if (x > 0)
                {
                    ...
                    if (left.noise < cell.noise)
                    {
                        ...
                        Vector3 a = new Vector3(topLeftX + x - sizePerBlock, currHeight, topLeftZ - y + sizePerBlock);
                        Vector3 b = new Vector3(topLeftX + x - sizePerBlock, currHeight, topLeftZ - y - sizePerBlock);
                        Vector3 c = new Vector3(topLeftX + x - sizePerBlock, currHeight - leftHeight, topLeftZ - y + sizePerBlock);
                        Vector3 d = new Vector3(topLeftX + x - sizePerBlock, currHeight - leftHeight, topLeftZ - y - sizePerBlock);
                ...
                if (x < size - 1)
                {
                    ...
                    if (right.noise < cell.noise)
                    {
                        ...
                        Vector3 a = new Vector3(topLeftX + x + sizePerBlock, currHeight, topLeftZ - y - sizePerBlock);
                        Vector3 b = new Vector3(topLeftX + x + sizePerBlock, currHeight, topLeftZ - y + sizePerBlock);
                        Vector3 c = new Vector3(topLeftX + x + sizePerBlock, currHeight - rightHeight, topLeftZ - y - sizePerBlock);
                        Vector3 d = new Vector3(topLeftX + x + sizePerBlock, currHeight - rightHeight, topLeftZ - y + sizePerBlock);
                ...
                if (y > 0)
                {
                    ...
                    if (down.noise < cell.noise)
                    {
                        ...
                        Vector3 a = new Vector3(topLeftX + x - sizePerBlock, currHeight, topLeftZ - y + sizePerBlock);
                        Vector3 b = new Vector3(topLeftX + x + sizePerBlock, currHeight, topLeftZ - y + sizePerBlock);
                        Vector3 c = new Vector3(topLeftX + x - sizePerBlock, currHeight - downHeight, topLeftZ - y + sizePerBlock);
                        Vector3 d = new Vector3(topLeftX + x + sizePerBlock, currHeight - downHeight, topLeftZ - y + sizePerBlock);
                ...
                if (y < size - 1)
                {
                    ...
                    if (up.noise < cell.noise)
                    {
                        ...
                        Vector3 a = new Vector3(topLeftX + x + sizePerBlock, currHeight, topLeftZ - y - sizePerBlock);
                        Vector3 b = new Vector3(topLeftX + x - sizePerBlock, currHeight, topLeftZ - y - sizePerBlock);
                        Vector3 c = new Vector3(topLeftX + x + sizePerBlock, currHeight - upHeight, topLeftZ - y - sizePerBlock);
                        Vector3 d = new Vector3(topLeftX + x - sizePerBlock, currHeight - upHeight, topLeftZ - y - sizePerBlock);
        ...
        DrawTextureChunk(mapaCells,pos,render);
    }
}